...
| A listing requires citizenship/clearance and no work-auth status confirmed yet | `eligibility` asks a one-time chat confirmation question (`route_work_auth_confirmation_reply` in `guardian.py` deterministically routes the reply back to `eligibility`, bypassing LLM routing for that turn) | Answer the question in chat; it is never inferred from a resume |
